...
const CUTS = [
  {
    ...
    desc: '거실 와이드 — 아이(2~3살)가 티렉스 메이트(30~40cm)를 꼭 안고 네이비 맥스를 향해 달려오는 도입. kling 보행 한계를 감안해 스틸 = "도착 직전 마지막 스텝" 순간으로 고정, 모션은 1~2스텝+상체 반동만. 로고 인트로는 1·2탄 방식(정중앙 오버레이 → 페이드아웃). 2탄 거실 연속성.',
    status: '⬜ 대기 — 아이 캐스팅 스틸이 첫 게이트 (3중 락: 2탄 C4 승인컷 얼굴참조 + baby Element + 2~3살 성장 프로즈)',
  },
  {
    id: 'C2', energy: '🏗 놀이 1', t: '3–6s',
    title: '쿠션 성 쌓기 — 아빠 재등장',
    motion: '아빠가 팟을 얹고, 아이가 꼭대기에 티렉스를 올림 — 둘이 마주보며 뿌듯한 웃음',
    desc: '아빠(1탄 Element 재등장)와 아이가 맥스·팟·서포트로 쿠션 요새를 쌓는 놀이 — 시선 릴레이 = 아이↔아빠 (2탄 C3 교훈: 컷의 감정은 시선으로 연결). 팟은 "plump, smooth undented top" 형태 락. 제품 3종 동시 노출 컷.',
    status: '⬜ 대기',
  },
  {
...
const CAST = [
  { role: '아이 (주연)', el: '신규 캐스팅 게이트', img: '/fam2/c4_winter_v3.png?v=1', imgNote: '성장 출발점 = 2탄 C4 돌쟁이(최신 승인 룩)', note: '2~3살 — 2탄 아기(ef781c67)의 성장형. 성장 편집 = 드리프트 최고위험 → 처음부터 3중 락(2탄 C4 승인컷 얼굴참조 + Element + 프로즈). 첫 스틸에서 룩 확정 후 신규 Element 락 검토' },
  ...
  { role: '제품', el: '레지스트리 단일 소스', note: '맥스 네이비(생성 단계 확정 — 딥 컬러 1순위) · 팟(undented top 락) · 스퀴지보 5.5cm 골프공 스케일문 · 메이트 티렉스 30~40cm · 지퍼 반대면 · 엄마 160cm 스케일 앵커 — 360·Element는 /products 레지스트리 참조' },
];
